let eredmenyek = [
  ...
  function TransformDatabase(array){
    let transformedData = [];
    for (let i=0; i<array.length; i++){
        let dataParts = array[i].split(" ");
        let actualObject = {
            helyezes: Number(dataParts[0]),
            CsapatMeret: Number(dataParts[1]),
            SportAg: dataParts[2],
            Versenyszam: dataParts[3]
        }
        transformedData.push(actualObject);
    }
...
async function UploadDatabase(){
    try{
        ...
        const database = TransformDatabase(eredmenyek);
        //console.log(database);
        const result = collection.insertMany(database);
        console.log("Sikeres feltöltés", result.insertedCount);
        client.close();
    }
    catch (error)
    {
        console.error("Hiba az adatok feltöltése során:", error);
    }
